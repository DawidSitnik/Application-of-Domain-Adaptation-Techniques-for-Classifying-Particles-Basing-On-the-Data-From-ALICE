...
class Classifier(nn.Module):
    ...
    def __init__(self, backbone: nn.Module, num_classes: int, bottleneck: Optional[nn.Module] = None,
                 bottleneck_dim: Optional[int] = -1, head: Optional[nn.Module] = None, classifier_type='other'):
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """"""
        if self.classifier_type == 'source':
            predictions, f = self.backbone(x)
        else:
            f = self.backbone(x)
            f = f.view(-1, Config.net_out_features)
            f = self.bottleneck(f)
